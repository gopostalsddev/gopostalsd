...
def test_get_all_users(client):
    ...
    user1 = User(
        first_name="Test", last_name="User", email="test1@testing.com",
        role_id=role.id, shipping_address_id=shipping_address.id,
        billing_address_id=billing_address.id
    )
    user2 = User(
        first_name="Another", last_name="User", email="test2@testing.com",
        role_id=role.id, shipping_address_id=shipping_address.id,
        billing_address_id=billing_address.id
    )
...
def test_delete_user_success(client):
    ...
    UserController.create_user(user_data)
    result = UserController.delete_user(user_data["email"])
...
def test_delete_user_invalid_email(client):
    result = UserController.delete_user("Invalid-email-format")

    assert isinstance(result, Result)
    assert result.status is False
    assert result.error == UserErrors.INVALID_EMAIL
    assert result.data is None
